...
if [ ! -e ${SCHED_FILE} ]; then
    echo "I/O スケジューラのファイル (${SCHED_FILE}) が存在しません" >&2
    exit 1
fi
...
if [ ! -e ${READ_AHEAD_KB_FILE} ]; then
    echo "readahead の設定ファイル (${READ_AHEAD_KB_FILE}) が存在しません" >&2
    exit 1
fi
...
for SCHED in ${SCHEDULERS}; do
    ...
    for READ_AHEAD_KB in 128 0; do
        ...
        echo "pttern: read, sched: ${SCHED}, read_ahead_kb: ${READ_AHEAD_KB}" >&2
...
for SCHED in ${SCHEDULERS}; do
    ...
    for READ_AHEAD_KB in 128 0; do
        ...
        awk -v sched=${SCHED} -v read_ahead_kb=${READ_AHEAD_KB} '{print sched, read_ahead_kb, $1}' <$FILENAME >>${OUTFIILENAME}
    done
done

# I/O スケジューラの効果確認のデータ採取
...
for SCHED in ${SCHEDULERS}; do
    ...
    for NUM_JOBS in ${JOB_PATTERNS}; do
        ...
        awk -v num_jobs=${NUM_JOBS} '{print num_jobs, $2, $3}' <$FILENAME >>${OUTFIILENAME}
    done
done
