...
1. 2^2(4)Kバイトから2^4.25Kバイト, 2^(4.5)Kバイト,...と、最終的には64Mバイトの数値に対して以下の処理をする
	...
	3. 1回のアクセス当たりの所要時間を記録
2. 1の結果をもとに chache.jpg というファイルにグラフを出力
...
const (
	CACHE_LINE_SIZE = 64
	// プログラムがうまく動作しなければこの値を変更する
	// 高速なマシンではアクセス数が足りずに、特にバッファサイズが小さい時の値がおかしいことがある
	// 低速なマシンなら時間がかかりすぎることがあるのでこの値を小さくする
...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "コマンドの実行に失敗しまいた: %q: %q", err, string(out))
		os.Exit(1)
	}	
}
